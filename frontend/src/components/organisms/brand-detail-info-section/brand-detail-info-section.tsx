...
import type { BrandDetailInfoContactItem, BrandDetailInfoSectionProps } from "./brand-detail-info-section.types";
...
function ContactIcon({ icon }: Pick<BrandDetailInfoContactItem, "icon">) {
...
export function BrandDetailInfoSection({
  className,
  contacts,
  description,
}: BrandDetailInfoSectionProps) {
  return (
    <section className={cn("brand-detail-info", className)}>
      <Container className="brand-detail-info__container">
        {description ? <p className="brand-detail-info__description">{description}</p> : null}

        <div className="brand-detail-info__contacts">
          {contacts.map((item) => {
            const content = (
              <>
                <span className="brand-detail-info__contact-icon">
                  <ContactIcon icon={item.icon} />
                </span>
                <span className="brand-detail-info__contact-label">{item.label}</span>
              </>
            );

            return item.href ? (
              <Link className="brand-detail-info__contact" href={item.href} key={item.id}>
                {content}
              </Link>
            ) : (
              <div className="brand-detail-info__contact" key={item.id}>
                {content}
              </div>
            );
          })}
